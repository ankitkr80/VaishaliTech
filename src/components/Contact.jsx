...
function Contact() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await fetch('https://v-backend-r6an.onrender.com/api/contact', {
        method: 'POST',
        headers: {
  ...
  return (
    <motion.div
      ...
      style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px' }} // Lighter background and padding fix
      variants={variants}
      initial="initial"
      whileInView="animate"
      viewport={{ once: true, amount: 0.005 }} // Triggers animation when 0.5% is in view
    >
        ...
        {!isSubmitted ? (
          ...
          >
            <input
              type="text"
              placeholder="Name"
              className="p-[10px] border border-gray-400 bg-white rounded-md text-black" // Text changed to black
